...
namespace efc_final
{
    ...
    public class SignUp : Activity, IOnClickListener, IOnCompleteListener
    {
        ...
        // выполняется после завершения регистрации firebase
        public async void OnComplete(Task task)
        {
            ...
            else
            {
                ...
                SignUp_PopUpCancelButton.Click += SignUp_PopUpCancelButton_Click;
                SignUp_PopUpTextView.Text = "Зарегистрироваться не получилось. Убедитесь, что ваш пароль не меньше 6 символов и введенная почта не была использована ранее.";
            }
        }
        ...
        private async Task<bool> SignInUser_New() 
        {
            ...
            try
            {
                ...
                if (!await SQLF.UploadData_NewUser(new List<string>() { uid,email,password })) { c = false; }
                else 
                { 
                    int id = Int32.Parse(SQLF.SQLF_answer_string);
                    User.Current = new User(uid,email,password);
                    User.Current.Id = id;
